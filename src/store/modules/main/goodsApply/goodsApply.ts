...
import { IGoodsApplyState, IPayLoadtype } from './type'
// 请求相关
import {
  getPageList,
  delPageList,
...
const noticeModule: Module<IGoodsApplyState, IRootState> = {
  namespaced: true,
  state() {
    return {
      goodsApplyList: [],
  ...
  mutations: {
    // 保存用户列表的数据
    saveGoodsApplyData(state, payLoad) {
      const { list, totalCount } = payLoad
      state.goodsApplyList = list
      state.goodsApplyCount = totalCount
    }
  },
  actions: {
    // 请求页面列表的数据
