...
import { IMaintenanceState, IPayLoadtype } from './type'
// 请求相关
import {
  getPageList,
  delPageList,
  addPageList,
  editPageList
} from '@/service/main/maintenance/maintenance'
// 工具类
import { showMessage } from '@/utils/message'
const noticeModule: Module<IMaintenanceState, IRootState> = {
  namespaced: true,
  state() {
    return {
      maintenanceList: [],
  ...
  mutations: {
    // 保存用户列表的数据
    saveMaintenanceData(state, payLoad) {
      const { list, totalCount } = payLoad
      state.maintenanceList = list
      state.maintenanceCount = totalCount
    }
  },
  actions: {
    // 请求页面列表的数据
