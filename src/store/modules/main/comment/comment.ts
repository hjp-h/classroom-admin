...
import { ICommentState, IPayLoadtype } from './type'
// 请求相关
import {
  getPageList,
  delPageList,
  addPageList,
  editPageList
} from '@/service/main/comment/comment'
// 工具类
import { showMessage } from '@/utils/message'
const noticeModule: Module<ICommentState, IRootState> = {
  namespaced: true,
  state() {
    return {
      commentList: [],
  ...
  mutations: {
    // 保存用户列表的数据
    saveCommentData(state, payLoad) {
      const { list, totalCount } = payLoad
      state.commentList = list
      state.commentCount = totalCount
    }
  },
  actions: {
    // 请求页面列表的数据
